...
contract TimeBrawlFactory is ITimeBrawlFactory, Initializable {
    ...
    function createBrawl(address base, address quote, uint256 startPrice_, address bet_, uint256 endTime_)
        external
        override
        returns (address orderbook)
    {
        ...
        address proxy = CloneFactory._createCloneWithSalt(impl, _getSalt(base, quote, id));
        ITimeBrawl(proxy).initialize(id, portal, startPrice_, bet_, endTime_);
    ...
    function _predictAddress(address base, address quote, uint256 id) internal view returns (address) {
        bytes32 salt = _getSalt(base, quote, id);
        return CloneFactory.predictAddressWithSalt(address(this), impl, salt);
    }

    function _getSalt(address base, address quote, uint256 id) internal pure returns (bytes32) {
