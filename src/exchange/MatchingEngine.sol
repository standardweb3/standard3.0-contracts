...
contract MatchingEngine is Initializable, ReentrancyGuard, AccessControl {
    ...
    struct OrderData {
        ...
        uint256 mp;
        /// next market price after matching
        uint256 np;
    ...
    function _detMarketBuyMakePrice(
        ...
        uint256 up;
        // lmp is the last matched price and askHead is the next price of lmp
        if (askHead == 0 && bidHead == 0) {
            ...
            if (lmp != 0) {
                // if bidHead is stored with stop order and ask orderbook is empty
                if (bidHead >= lmp) {
                    up = (lmp * (10000 + spread)) / 10000;
                    return up;
                } else {
            ...
            if (lmp != 0) {
                ...
                if (askHead >= up) {
                    ...
                    // if askHead is stored with stop order and bid orderbook is empty
                    if(askHead < lmp) {
                        return up;
                    }
                    // if ask head is just lower than up
                    return askHead;
                }
            }
            return askHead;
    ...
    function _detMarketSellMakePrice(
        ...
        if (askHead == 0 && bidHead == 0) {
            ...
            if (lmp != 0) {
                down = (lmp * (10000 - spread)) / 10000;
                if (down <= bidHead) {
                    return bidHead > lmp ? down : bidHead;
                } else {
                    return down == 0 ? 1 : down;
                }
            }
    ...
    function stopBuyETH(
        ...
        IWETH(WETH).deposit{value: msg.value}();
        return
            stopBuy(base, WETH, price, msg.value, isMaker, n, uid, recipient);
    }
    ...
    function stopSellETH(
        ...
        IWETH(WETH).deposit{value: msg.value}();
        return
            stopSell(WETH, quote, price, msg.value, isMaker, n, uid, recipient);
    }
        ...
        )
    {
        ...
        // In LimitBuy
        if (isBid) {
            // if last market price is 0, there is no order
            if (lmp != 0) {
                if (askHead != 0 && limitPrice < askHead) {
                    return (remaining, bidHead, askHead, lmp);
                } else if (askHead == 0) {
                    return (remaining, bidHead, askHead, lmp);
                }
            }

            // set askHead above or same to bidHead to skip stop sell orders in bid orderbook price range
            while (askHead < lmp && askHead != 0) {
                askHead = IOrderbook(orderbook).nextPrice(false, askHead);
            }
        ...
        // In LimitSell
            ...
            // set bidHead below or same to askHead to skip stop buy orders in ask orderbook price range
            while (bidHead > lmp && bidHead != 0) {
                bidHead = IOrderbook(orderbook).nextPrice(true, bidHead);
            }
        ...
        return (remaining, bidHead, askHead, lmp); // return bidHead, and askHead
    }
