...
// We have some testing src that purposely cause unreachable code, but said code is in the library src, and
...
task(TASK_COMPILE_SOLIDITY_GET_COMPILATION_JOB_FOR_FILE, async (params, _, runSuper) => {
  ...
  if (params.file.sourceName.startsWith('src/mocks/') && /\bunreachable\b/.test(params.file.sourceName)) {
