...
* src not intended to be used standalone should be marked abstract
  so they are required to be inherited to other src.
...
* Custom errors should be declared following the [EIP-6093](https://eips.ethereum.org/EIPS/eip-6093) rationale whenever reasonable. Also, consider the following:
  ...
  * The location of custom errors should be decided in the following order:
    ...
    4. Declare the error in an extension if the error only happens in such extension or child src.

  * Custom error names should not be declared twice along the library to avoid duplicated identifier declarations when inheriting from multiple src.
