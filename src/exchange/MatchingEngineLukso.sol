...
contract MatchingEngine is ReentrancyGuard, AccessControl {
    ...
    event PairAdded(
        address orderbook, TransferHelper.TokenInfo base, TransferHelper.TokenInfo quote, uint256 listingDate
    );
        ...
        returns (address book)
    {
        ...
        emit PairAdded(orderbook, baseInfo, quoteInfo, listingDate);
    ...
    function _detMake(
        ...
        if (remaining > 0) {
            ...
            TransferHelper.lsp7Transfer(isBid ? quote : base, address(this), stopTo, remaining);
            if (isMaker) {
        ...
        returns (uint256 withoutFee, address pair)
    {
        ...
        if (isBid) {
            ...
            if (quote != WETH) {
                TransferHelper.lsp7Transfer(quote, msg.sender, address(this), amount);
            }
            TransferHelper.lsp7Transfer(quote, address(this), feeTo, fee);
        } else {
            // transfer input asset give user to this contract
            if (base != WETH) {
                TransferHelper.lsp7Transfer(base, msg.sender, address(this), amount);
            }
    ...
    function _listingDeposit(address payment, address sender) internal {
        ...
        if (payment != WETH) {
            TransferHelper.lsp7Transfer(payment, msg.sender, address(this), amount);
        }
