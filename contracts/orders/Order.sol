...
contract Order is Initializable, IOrder {
    ...
    function execute(address sender, uint256 amount) public onlyOrderBook {
        ...
        (string memory _pairName, uint256 _mktPrice, address base, address quote) = IOrderbook(orderbook).pairInfo();
        ...
        if (isAsk) {
            ...
            require(verifyRatio(base, deposit, IERC20(base).balanceOf(address(this)), amount), "Price is not correct");
        ...
        // if the order is bid order on the base/quote pair
            ...
            // verify whether the claim is correct with converted value from the ratio with error allowance of 0.01%
            require(verifyRatio(deposit, quote, amount, IERC20(quote).balanceOf(address(this))), "Price is not correct");
    ...
    function verifyRatio(address base, address quote, uint256 baseAmount, uint256 quoteAmount) public view returns (bool success) {
        // get pair info and decimals for each asset
        uint256 bDecimal = IERC20(base).decimals();
        uint256 qDecimal = IERC20(quote).decimals();
        uint256 claimedPrice = (baseAmount * (10**qDecimal)) * 1e8 / (quoteAmount * (10**bDecimal));
        // verify whether the claim is correct with converted value from the ratio with error allowance of 0.01%
