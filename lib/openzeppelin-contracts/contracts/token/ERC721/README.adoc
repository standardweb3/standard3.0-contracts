...
[.readme-notice]
NOTE: This document is better viewed at https://docs.openzeppelin.com/src/api/token/erc721

This set of interfaces, src, and utilities are all related to the https://eips.ethereum.org/EIPS/eip-721[ERC721 Non-Fungible Token Standard].
...
* {IERC721Receiver}: An interface that must be implemented by src if they want to accept tokens through `safeTransferFrom`.

OpenZeppelin src provides implementations of all four interfaces:
...
NOTE: This core set of src is designed to be unopinionated, allowing developers to access the internal functions in ERC721 (such as <<ERC721-_mint-address-uint256-,`_mint`>>) and expose them as external functions in the way they prefer.
