...
// OpenZeppelin src (last updated v5.0.0) (governance/extensions/GovernorTimelockAccess.sol)
...
abstract contract GovernorTimelockAccess is Governor {
    ...
    // For all other target src, the manager is used by default, and a true toggle means it will be ignored.
     ...
     *
     * NOTE: Execution delays are processed by the `AccessManager` src, and according to that contract are
