...
contract MatchingEngine2 is AccessControl, Initializable, UUPSUpgradeable {
    // roles
    ...
    function cancelOrder(
        ...
        (uint256 remaining, address base, address quote) = IOrderbook2(orderbook)
    ...
    function getPrices(
        ...
        return IOrderbook2(orderbook).getPrices(isAsk, n);
    }
    ...
    function getOrders(
        ...
        address orderbook = getBookByPair(base, quote);
        return IOrderbook2(orderbook).getOrders(isAsk, price, n);
    }
    ...
    function getOrder(
        ...
        address orderbook = getBookByPair(base, quote);
        return IOrderbook2(orderbook).getOrder(isAsk, orderId);
    }
    ...
    function getOrderIds(
        ...
        address orderbook = getBookByPair(base, quote);
        return IOrderbook2(orderbook).getOrderIds(isAsk, price, n);
    }
    ...
    function mktPrice(
        ...
        address orderbook = getBookByPair(base, quote);
        return IOrderbook2(orderbook).mktPrice();
    }
    ...
    function convert(
        ...
        if (base == quote) {
            ...
            return IOrderbook2(orderbook).assetValue(amount, isAsk);
        }
    }
    ...
    function _stopOrder(
        ...
        // create order
        if (isAsk) {
            IOrderbook2(orderbook).placeAsk(msg.sender, at, withoutFee);
        } else {
            IOrderbook2(orderbook).placeBid(msg.sender, at, withoutFee);
        }
    }
    ...
    function _matchAt(
        ...
        while (
            remaining > 0 &&
            !IOrderbook2(orderbook).isEmpty(!isAsk, priceAt) &&
            i < n
        ) {
            // fpop OrderLinkedList by price, if ask you get bid order, if bid you get ask order
            uint256 orderId = IOrderbook2(orderbook).fpop(!isAsk, priceAt);
            // Get quote asset on bid order on buy, base asset on ask order on sell
            uint256 required = IOrderbook2(orderbook).getRequired(
                !isAsk,
                priceAt,
                orderId
            );
            ...
            if (remaining <= required) {
                ...
                address owner = IOrderbook2(orderbook).execute(
                    orderId,
                    !isAsk,
                    priceAt,
                    msg.sender,
    ...
    function _limitOrder(
        ...
        if (isAsk) {
            ...
            uint256 bidHead = IOrderbook2(orderbook).bidHead();
            while (
            ...
            while (
                ...
                askHead = IOrderbook2(orderbook).askHead();
            }
        }
        // set last match price
        if (lmp != 0) {
            IOrderbook2(orderbook).setLmp(lmp);
        }
        return (remaining);
    }
