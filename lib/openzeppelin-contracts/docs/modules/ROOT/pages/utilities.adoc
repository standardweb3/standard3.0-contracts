...
The OpenZeppelin src provide a ton of useful utilities that you can use in your project. Here are some of the more popular ones.
...
In Solidity, it's frequently helpful to know whether or not a contract supports an interface you'd like to use. ERC165 is a standard that helps do runtime interface detection. src provide helpers both for implementing ERC165 in your src and querying other src:
...
Although Solidity already provides math operators (i.e. `+`, `-`, etc.), src includes xref:api:utils.adoc#Math[`Math`]; a set of utilities for dealing with mathematical operators, with support for extra operations (eg. xref:api:utils.adoc#Math-average-uint256-uint256-[`average`]) and xref:api:utils.adoc#SignedMath[`SignedMath`]; a library specialized in signed math operations.

Include these src with `using Math for uint256` or `using SignedMath for int256` and then use their functions in your code:
...
Some use cases require more powerful data structures than arrays and mappings offered natively in Solidity. src provides these libraries for enhanced data structure management:
